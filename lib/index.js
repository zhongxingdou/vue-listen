...
// a.2.c.2.3 => a[2].c[2][3]
function normalizeNumberMember (path) {
...
function deleteMapPath (map, path) {
  let info = map[path]
  info && info.unWatch && info.unWatch()
...
function buildWatchHandler (deepWatchOption) {
  ...
  return function (val, oldVal) {
    ...
    deepWatchOption.handler.call(this, val, oldVal, path)
  }
}
...
function deepWatch(option) {
  ...
  if (!onlyDescendence) {
    let watchTarget = path.indexOf('[') >= 0
      ? new Function(`try{return this.${normalizeNumberMember(path)}}catch(e){return undefined}`)
      : path
  ...
  install: function (vue, option = {}) {
    ...
    vue.prototype.$listen = function (exp, handler, option = {}) {
      ...
      if (deep) {
        ...
        let val = vm.$get
          ? vm.$get(exp)
          : (new Function('return this.' + normalizeNumberMember(exp))).call(vm, exp)
